...
bool queue_insert_head(queue_t *q, const char *s) {
    ...
    // allocate space for string vale in list element
...
bool queue_remove_head(queue_t *q, char *buf, size_t bufsize) {
    ...
    // extract the head aims to remove
    list_ele_t *oldh = q->head;
    char *s = oldh->value;

    // manually copy to buf
    if (buf && bufsize > 0) {
        size_t len = strlen(s);
        // at most copy bufsize - 1 characters, and add a null terminator at the
        // end
        if (len >= bufsize)
            len = bufsize - 1;
        memcpy(buf, s, len);
